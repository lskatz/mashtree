...
sub main{
  ...
  my $tsv = mashDistance($sketches,\@reads,$$settings{tempdir},$settings);
...
sub clusterDijkstra{
  ...
  my %C; #clustering, where the key is the seed

  logmsg "Optimizing distances with Dijkstra";
  my $graph = Graph::Dijkstra->new(edgedefault=>"undirected");
  open(my $tsvFh, $tsv) or die "ERROR: could not read $tsv: $!";
  while(<$tsvFh>){
    chomp;
    my($g1,$g2,$dist)=split /\t/;
    next if($g1 eq $g2);
...
sub mashDistance{
  ...
  my $tsv = "$outdir/distances.tsv";
  logmsg "Converting to tsv format into $tsv";
  open(my $tsvFh, ">", $tsv) or die "ERROR: could not write to $tsv: $!";
  print $tsvFh scalar($mashtreeDb->toString(\@genomeName,"tsv"));
  close $tsvFh;
...
sub usage{
  "$0: use distances from Mash (min-hash algorithm) to cluster genomes
  Usage: $0 [options] *.fastq *.fasta *.gbk *.msh > clusters.tsv
