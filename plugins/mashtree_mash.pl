...
sub main{
  my $settings={};
  GetOptions($settings,qw(help tempdir=s numcpus=i genomesize=i mindepth|min-depth=i truncLength=i kmerlength=i sort-order=s sketch-size=i version)) or die $!;
  $$settings{numcpus}||=1;
  ...
  # Parse which parameters are reads or the database
  # Exactly only one database allowed though.
  my($db, @reads);
  for(@ARGV){
  ...
  mashDistance($sketches,\@reads,$db,$settings);
...
sub sketchAll{
  ...
  # Balancing by file size is probably a good way to
  # balance the threads.
...
sub mashDistance{
  my($mshList,$reads,$dbFilename,$settings)=@_;
  ...
  my $mashtreeDb=Mashtree::Db->new($dbFilename);
